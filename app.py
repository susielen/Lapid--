...
if arquivo:
    with st.spinner('💎 O Mestre está lapidando...'):
        try:
            time.sleep(1)
            ...
            if banco:
                ...
                with pd.ExcelWriter(out, engine='xlsxwriter') as writer:
                    ...
                    f_vde = wb.add_format({'num_format': 'R$ #,##0.00', 'font_color': 'green', 'border': 1})
                    f_vrm = wb.add_format({'num_format': 'R$ #,##0.00', 'font_color': 'red', 'border': 1})
                    f_cab = wb.add_format({'bold':1,'bg_color':'#F2F2F2','border':1, 'align':'center'})
                    for cod, df in banco.items():
                        ws = wb.add_worksheet(str(cod)[:31])
                        ws.set_column('G:H', 1); ws.set_column('D:D', 35)
                        ...
                        ws.write(rf, 10, "Saldo Final:", f_cab); ws.write(rf, 11, s, f_vde if s >= 0 else f_vrm)
